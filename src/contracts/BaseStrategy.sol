...
abstract contract BaseStrategy {
    ...
    function _initialize(
        ...
        IERC20(address(vault)).approve(rewards, type(uint256).max); // Allow rewards to be pulled
    }
    ...
    function setRewards(address _rewards) external {
        ...
        IERC20(address(vault)).approve(rewards, 0);
        rewards = _rewards;
        IERC20(address(vault)).approve(rewards, type(uint256).max);
     ...
     */
    function prepareReturn(uint256 _debtOutstanding, bytes memory params)
        internal
        virtual
        returns (
            uint256 _profit,
     ...
     */
    function adjustPosition(uint256 _debtOutstanding, bytes memory params)
        internal
        virtual;
     ...
     */
    function harvest(bytes memory params) external {
        _onlyKeepers();
        uint256 profit = 0;
        uint256 loss = 0;
        ...
        if (emergencyExit) {
            ...
            (profit, loss, debtPayment) = prepareReturn(
                debtOutstanding,
                params
            );
        }
        ...
        adjustPosition(debtOutstanding, params);
